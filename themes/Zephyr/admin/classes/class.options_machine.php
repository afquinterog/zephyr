...
class Options_Machine {
	...
	public static function optionsframework_machine($options) {
		...
		foreach ($options as $value) {
			...
			if ($value['type'] == 'multicheck'){
				...
				if (isset($value['id']) AND isset($value['std'])) $defaults[$value['id']] = $value['std'];
			}
			...
			if (
				! in_array($value['type'], array('info', 'image', 'heading', 'slider', 'sorter', 'backup', 'transfer')) AND
				! isset($smof_data[$value['id']]) AND
				isset($defaults[$value['id']])){
	...
	public static function optionsframework_uploader_function($id,$std,$mod){
		...
		$uploader .= '<div class="upload_button_div"><span class="button image_upload_button" id="' . $id . '" data-uploading="' . __( 'Uploading', 'us' ) . '" data-upload="' . __( 'Upload', 'us' ) . '">' . __( 'Upload', 'us' ) . '</span>';

		if(!empty($upload)) {$hide = '';} else { $hide = 'hide';}
		$uploader .= '<span class="button image_reset_button '. $hide.'" id="reset_'. $id .'" title="' . $id . '">'.__( 'Remove', 'us' ).'</span>';
		$uploader .='</div>' . "\n";
