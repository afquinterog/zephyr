...
add_action( 'admin_menu', 'us_add_demo_import_page', 15 );
function us_add_demo_import_page() {
	add_submenu_page( 'us-home', __( 'Demo Import', 'us' ), __( 'Demo Import', 'us' ), 'manage_options', 'us-demo-import', 'us_demo_import' );
}
...
function us_demo_import() {
	...
	<div class="w-message content" style="display:none;">
		<img src="<?php echo $us_template_directory_uri ?>/functions/assets/img/spinner.gif" alt="spinner">
		...
		<p class="w-message-text">
			...
			<?php _e( 'This can take a while if your server is slow (inexpensive hosting).', 'us' ) ?><br>
			<?php _e( 'You will be notified via this page when the import is completed.', 'us' ) ?></p>
	</div>

	<div class="w-message success" style="display:none;">
		<h1 class="w-message-title"><?php _e( 'Import completed successfully!', 'us' ) ?></h1>
			...
			<div class="w-importer-list">
				<?php foreach ( $config as $name => $import ): ?>
					<div class="w-importer-item">
						...
						<label class="w-importer-item-preview" for="demo_<?php echo $name; ?>">
							<h2 class="w-importer-item-title"><?php echo $import['title']; ?></h2>
							<img src="<?php echo $us_template_directory_uri . '/' . $import['image']; ?>" alt="<?php echo $import['title']; ?>">
						</label>
		...
		<div class="w-importer-options">
			...
			<div class="w-importer-option rev-slider">
				<label class="w-importer-option-check">
					...
					if ( 0 < count( $sliders_avaliable_for ) AND count( $sliders_avaliable_for ) < count( $config ) ) {
						?><span class="w-importer-option-desc">(
						<?php echo sprintf( __( 'available for %s only', 'us' ), implode( ', ', $sliders_avaliable_for ) ) ?>
						)</span><?php
					}
					?>
				</label>
			...
			<?php if ( count( $sliders_avaliable_for ) > 0 AND ! class_exists( 'RevSlider' ) ): ?>
				<div class="w-importer-note"><p>
						<?php _e( '<strong>Revolution Slider</strong> plugin is not active. Please activate it if you want Sliders to be imported.', 'us' ) ?>
					</p></div>
			<?php endif; ?>
...
function us_demo_import_options() {
	...
	$smof_data = unserialize( base64_decode( file_get_contents( $us_template_directory . '/demo-import/' . $demo_version . '-options.txt' ) ) ); //100% safe - ignore theme check nag
