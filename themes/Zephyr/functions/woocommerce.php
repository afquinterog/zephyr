...
if ( ! function_exists( 'us_woocommerce_before_main_content' ) ) {
	...
	function us_woocommerce_before_main_content() {
		echo '<div class="l-main"><div class="l-main-h i-cf"><div class="l-content g-html">';
 ...
 *
 */
add_filter( 'woocommerce_related_products_args', 'us_woocommerce_related_products_args' );
function us_woocommerce_related_products_args() {
	global $product;

	$args = array(
		'post_type' => 'product',
		'no_found_rows' => 1,
		'posts_per_page' => us_get_option( 'product_related_qty', 4 ),
...
function us_woo_fix_product_lightbox( $html ) {
	...
	$html = preg_replace( '~ class="([^"]+ )?zoom( [^"]+)?"~u', ' class="$1$2"', $html );
	$html = preg_replace( '~ data-rel="prettyPhoto[^"]+?"~u', '', $html );

	// sprintf( '<a href="%s" itemprop="image" class="woocommerce-main-image zoom" title="%s" data-rel="prettyPhoto' . $gallery . '">%s</a>', $image_link, $image_caption, $image );
	return $html;
}
